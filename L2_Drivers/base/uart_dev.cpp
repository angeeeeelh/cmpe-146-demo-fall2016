...
bool UartDev::clearTx(void)
{
	//mpUARTRegBase->FCR = (1 << 2); // bit is self clearing RX Fifo clear
	return xQueueReset(mTxQueue);
//    if (!mTxQueue) {
//        return false;
//    }
//    else {
...
bool UartDev::clearRx(void)
{
	//mpUARTRegBase->FCR = (1 << 1); // bit is self clearing TX Fifo clear
	return  xQueueReset(mRxQueue);
...
bool UartDev::getChar(char* pInputChar, unsigned int timeout)
{
    if (!pInputChar || !mRxQueue) {
        return false;							// if either the queue or input char do not exist then exit program
    }
