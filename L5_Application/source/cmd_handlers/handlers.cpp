...
CMD_HANDLER_FUNC(txuart2Handler)
{
	scheduler_task *task1 = scheduler_task::getTaskPtrByName("uart");

	return true;
}


CMD_HANDLER_FUNC(gpsHandler)
{

	bool start = cmdParams.beginsWithIgnoreCase("start");
	bool suspend = cmdParams.beginsWithIgnoreCase("suspend");

	scheduler_task *task1 = scheduler_task::getTaskPtrByName("gps");
...
CMD_HANDLER_FUNC(producerSuspendHandler)
{
	vTaskSuspend(NULL);
	return true;
}

CMD_HANDLER_FUNC(consumerSuspendHandler)
{
	vTaskSuspend(NULL);
	return true;
}
...
CMD_HANDLER_FUNC(orientationCmd)
{
    ...
    if (cmdParams == "on") {
        ...
        vTaskSuspend(task1->getTaskHandle()); // Can also use: compute->suspend();
    }
