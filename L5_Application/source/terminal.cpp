...
terminalTask::terminalTask(uint8_t priority) :
        scheduler_task("terminal", 1024*8, priority),
...
//                                           "'i2c discover' : Discovers all I2C devices on the BUS");
    ...
    cp.addHandler(gpsHandler,     "gps",    "Used to start task");



    cp.addHandler(txuart2Handler,     "senduart2",    "Used to send uart2 message");
    ...
    cp.addHandler(producerSuspendHandler, "producer", "Suspends producer task");
    cp.addHandler(consumerSuspendHandler, "consumer", "Suspends consumer task");
